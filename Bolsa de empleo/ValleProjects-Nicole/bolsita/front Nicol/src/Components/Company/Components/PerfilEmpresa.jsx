    ...
    Swal.fire({
      ...
      html: `
        <div style="display: flex; flex-direction: column; gap: 10px; align-items: center;">
          <div id="logoPreview" style="display: flex; justify-content: center; margin-bottom: 20px;">
            ${logo ? `<img src="${logo}" alt="Vista previa del logotipo" style="width: 100px; height: 100px; object-fit: cover; border-radius: 50%;">` : ""}
          </div>
          <label for="logoFile" style="font-weight: bold;">Logotipo:</label>
          <input type="file" id="logoFile" class="swal2-file-input" accept="image/*">
          ...
          <input type="text" id="telefono" class="swal2-input" placeholder="Teléfono (solo números)" value="${companyInfo.telefono}" inputmode="numeric">
      ...
      preConfirm: () => {
        ...
        const logoFile = Swal.getPopup().querySelector("#logoFile").files[0];
        return { razonSocial, sector, telefono, nombreEmpresa, ubicacion, logoFile };
      },
    }).then((result) => {
      if (result.isConfirmed) {
        const token = localStorage.getItem("token");
        const { logoFile, ...updatedInfo } = result.value;

        // Actualizar información de la empresa
        axios
          .put("http://localhost:5000/api/company/perfil", updatedInfo, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
        ...
        if (logoFile) {
          ...
          formData.append("imagenPerfil", logoFile);

          axios
            .put("http://localhost:5000/api/company/perfil/imagen", formData, {
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="max-w-7xl mx-auto bg-white dark:bg-gray-600 p-4 md:p-6 rounded-xl shadow-xl mt-12 ml-64 flex flex-col items-center space-y-6">
      ...
      <div className="relative mb-6">
        <div className="w-48 h-48 md:w-64 md:h-64 bg-white dark:bg-gray-600 rounded-full overflow-hidden shadow-lg">
          {logo ? (
            <img src={logo} alt="Logo de la empresa" className="w-full h-full object-cover rounded-full" />
          ) : (
            <div className="w-48 h-48 md:w-64 md:h-64 bg-white dark:bg-gray-600 rounded-full overflow-hidden shadow-lg flex items-center justify-center">
