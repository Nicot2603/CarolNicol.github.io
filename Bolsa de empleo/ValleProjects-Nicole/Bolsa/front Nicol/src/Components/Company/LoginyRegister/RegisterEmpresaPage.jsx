...
const RegisterEmpresaPage = ({ onRegisterEmpresa }) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      // Hacer la solicitud al backend para registrar la empresa usando axios
      const response = await axios.post("http://localhost:5000/api/auth/register/company", empresaData);
      ...
      if (response.status === 200) {
        ...
        setEmailError(response.data.message || "Error al registrar la empresa.");
        Swal.fire({
          icon: "error",
          title: "Error de registro",
          text: response.data.message || "Error al registrar la empresa.",
          confirmButtonText: "Aceptar",
          confirmButtonColor: "#3b82f6",
        });
      }
